			...
			if (mineList[0][a] > 0)
			{
				for (int b = 0; b < mineList[0][a]; b++)
				{
					...
					if (gob != null)//
					{
						...
						gob.GetComponent<bottomScript>().setPoint(a, GlobalDataScript.roomVo.guiPai);//设置指针                                
						SetPosition(false);
	...
	*/
	...
	public void moPai() //摸牌
	{
		...
		if (pickCardItem != null) //有可能没牌了
		{
			...
			pickCardItem.GetComponent<bottomScript>().setPoint(MoPaiCardPoint, GlobalDataScript.roomVo.guiPai); //得到索引
...
#pragma warning restore CS0219 // 变量“pengOrNot”已被赋值，但从未使用过它的值
	...
	private void hupaiCallBack(ClientResponse response){
		...
		if (GlobalDataScript.hupaiResponseVo.type == "0") {
			...
			if (GlobalDataScript.roomVo.roomType == GameConfig.GAME_TYPE_ZHUANZHUAN 
                || GlobalDataScript.roomVo.roomType == GameConfig.GAME_TYPE_CHANGSHA)
            {//转转麻将显示抓码信息
    ...
    */
    ...
    /**显示桌面鬼牌**/
    private void displayTouzi(int touzi, int gui)
    {
        if (gui != -1 && GlobalDataScript.roomVo.roomType == 4 && GlobalDataScript.roomVo.gui == 2)
        { //显示骰子
            int r1 = touzi / 10;
            int r2 = touzi % 10;
