...
public class ChatSocket{
	...
	public void Connect()
	{
		...
		try
		{
			...
			tcpclient.BeginConnect(IPAddress.Parse(APIS.chatSocketUrl), APIS.chatPort, new AsyncCallback(ConnectCallback), tcpclient);
	...
	{
        init = false;
        MyDebug.Log("--chatSocketUrl-------DisConnect断开连接-----");
		if (tcpclient != null)
		{
			tcpclient.Close();
			tcpclient = null;
	...
	private void TCPReadCallBack(IAsyncResult ar)
	{
		...
		{
            init = false;
            MyDebug.Log("--chatSocketUrl-------TCPReadCallBack-----客户端被动断开聊天服务器---");
			//被动断开时 
			mas.Close();
			state.client.Close();
			mas = null;
